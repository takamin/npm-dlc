...
const urlToQueryLatestVersion = packageName =>
 ...
 *
 * @param {string} packageName A package name.
 * @returns {string|null} The latest version.
 */
NpmPackage.getLatestVersion = async packageName => {
    const url = urlToQueryLatestVersion(packageName);
    try {
        const version = await getJson(url);
        return version.latest;
    } catch(err) {
        console.error(`Error in NpmPackage.getLatestVersion: ${err.message}`);
    }
    return null;
};
 ...
 * @async
 * @param {string} url an URL to request.
 * @returns {Promise<object>} HTTP response body of the URL.
 */
